...
class RaydiumSwap {
  ...
  constructor(RPC_URL: string, WALLET_PRIVATE_KEY: string) {
    ...
    this.wallet = new Wallet(
      Keypair.fromSecretKey(base58.decode(WALLET_PRIVATE_KEY)),
    );
  }
  ...
  async loadPoolKeys() {
    try {
      if (existsSync('pools.json')) {
        this.allPoolKeysJson = JSON.parse(
          (await readFile('pools.json')).toString(),
        );
        return;
      }
  ...
  async findRaydiumPoolInfo(
    ...
    const item = await this.connection.getAccountInfo(pool.marketId);
    if (!item) return null;
    const market = {
